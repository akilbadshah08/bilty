            ...
            paging: function(meta, callback) {
                var pg = {
                    ...
                    populate: function() {
                        ...
                        if (pageSizes.length == 0) pageSizes = [10, 50, 100, 500, 1000];
...
$(document).ready(function() {
    ...
    $('.co_applicatant').on('click', function() {
        $('.spouse,.father,.mother,.son,.son-s-wife').addClass('hide');
        $('.' + $(this).find('input').val()).removeClass('hide');
    ...
    $('.runningloan').click(function() {
        var next = $(this).attr('nextrow');
        $('.trow-' + next).removeClass('hide');
        $(this).addClass('hide');
    })

    $('#property_part_div').hide();
    $('#mother_deed_div').hide();
    $('#bank_name_div').hide();

    $('#property_type').change(function() {
        var property_type = $(this).val();
        if (property_type == 'bunglow' || property_type == 'plot') $('#property_part_div').show();
        else {
            $('#property_part_div').hide();
            $('#property_part').val('').trigger('change');
        }
        if ($(this).val() == 'plot plus construction') {
            $('#hl_purchase_construction_cost').show();
        } else {
            $('#hl_purchase_construction_cost').hide();
        }
    })
    $('#property_part').change(function() {
        var part_property = $(this).val();
        if (part_property == 'yes') $('#mother_deed_div').show();
        else {
            $('#mother_deed_div').hide();
        }
    })

    $('#seller_bt').change(function() {
        var bt = $(this).val();
        if (bt == 'yes') $('#bank_name_div').show();
        else {
            $('#bank_name_div').hide();
        }
    })

    $('body').on('click', '.coapplicantslist .delete-coapplicant', function(e) {
        e.preventDefault();
        if (confirm('Are you sure?')) {
            $(this).parent().parent().remove();
        }
    ...
    $('.state').on('change', function() {
        var city = $(this).attr('related');
        $('.' + city).html('<option>Select...</option>');
        $.ajax({
            method: "get",
            url: site_url + 'api/common/get_cities',
            data: { 'state_id': $(this).val() },
            success: function(result) {
                var cities = result.data;
                for (var i = 0; i < cities.length; i++) {

                    $('.' + city).append('<option value="' + cities[i].city_id + '">' + cities[i].city_name + '</option>');
                }
            }
        });
    })
    ...
    $('#property_part').trigger('change');
    $('#seller_bt').trigger('change');

    $('.emi_paid,.total_emi').change(function() {
        var tr = $(this).parent().parent();

        var unpaid = parseInt($(tr).find('.total_emi').val()) - parseInt($(tr).find('.emi_paid').val());
        if (unpaid != 'NaN') {
            $(tr).find('.unpaid_emi').val(unpaid).attr('readonly', '');
        }

    })
    $('.addapplicant').click(function() {
        $('#applicant_id').val('');
        $('#add_co_applicant input, #add_co_applicant select').removeClass('verror');
        $('.ediv').remove();
        $('.alert-danger').hide();
    })
    $('body').on('click', '#loan .coapplicant_button_id', function() {
        $('.addapplicant').click();
        $('#add_co_applicant input, #add_co_applicant select').removeClass('verror');
        $('.ediv').remove();
        $('.alert-danger').hide();
        var applicant_id = $(this).attr('aid');
        $('#applicant_id').val(applicant_id);
        if (applicant_id != '') {
            $('.add_co_applicant #input[name="customer[customer_email_id]"]').prop("disabled", true);
        } else {
            $('.add_co_applicant input[name="customer[customer_email_id]"]').prop("disabled", false);
        }
        $.ajax({
            type: "GET",
            url: site_url + 'api/customer/edit/' + applicant_id,
                ...
                if (data.status == "true") {
                    ...
                    for (var key in customer) {
                        if (typeof customer[key] === "object") {
                            innserarray = customer[key];
                            for (var innerkey in innserarray) {
                                $('#add_co_applicant  input[name="customer[' + key + '][' + innerkey + ']"][value="' + customer[key][innerkey] + '"]').prop('checked', true);
                                $('#add_co_applicant  select[name="customer[' + key + '][' + innerkey + ']"],#add_co_applicant  input[name="customer[' + key + '][' + innerkey + ']"],#add_co_applicant  textarea[name="[' + key + '][' + innerkey + ']"],#add_co_applicant  email[name="[' + key + '][' + innerkey + ']"]').val(customer[key][innerkey]);

                            }
                        } else {
                            $('#add_co_applicant  input[name="customer[' + key + ']"][value="' + customer[key] + '"]').prop('checked', true);
                            $('#add_co_applicant  select[name="customer[' + key + ']"],#add_co_applicant  input[name="customer[' + key + ']"],#add_co_applicant  textarea[name="customer[' + key + ']"],#add_co_applicant  email[name="customer[' + key + ']"]').val(customer[key]);

                        }

                    }
                    // $('select[name="customer[customer_city]"]').select2("val", customer['customer_city']);
                    //  $('select[name="customer[customer_state]"]').select2("val", customer['customer_state']);
    ...
    $('.amount').keyup(function() {
        ...
        $(this).parent().find('.words').html(inWords($(this).val()))
    })
    $('#loan #customer_id').change(function() {
        if ($(this).val() != '') {
            $('#loan input[name="customer[customer_email_id]"]').prop("disabled", true);
        } else {
            $('#loan input[name="customer[customer_email_id]"]').prop("disabled", false);
        }
        $.ajax({
            type: "GET",
            url: site_url + 'api/customer/edit/' + jQuery(this).val(),
            success: function(data) {
                //var data = JSON.parse(data);
                if (data.status == "true") {
                    var customer = data.data;
                    for (var key in customer) {
                        if (typeof customer[key] === "object") {
                            innserarray = customer[key];
                            for (var innerkey in innserarray) {
                                $('#loan select[name="customer[' + key + '][' + innerkey + ']"],#loan input[name="customer[' + key + '][' + innerkey + ']"],#loan textarea[name="[' + key + '][' + innerkey + ']"],#loan email[name="[' + key + '][' + innerkey + ']"]').val(customer[key][innerkey]);
                                $('#loan  input[name="customer[' + key + '][' + innerkey + ']"][value="' + customer[key][innerkey] + '"]').prop('checked', true);
                            }
                        } else {
                            $('#loan select[name="customer[' + key + ']"],#loan input[name="customer[' + key + ']"],#loan textarea[name="customer[' + key + ']"],#loan email[name="customer[' + key + ']"]').val(customer[key]);
                            $('#loan  input[name="customer[' + key + ']"][value="' + customer[key] + '"]').prop('checked', true);
                        }
